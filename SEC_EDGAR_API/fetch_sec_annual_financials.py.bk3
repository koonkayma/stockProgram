...
# --- Helper Functions --- (camel_to_snake, get_sec_data, parse_date, safe_decimal, safe_int_or_bigint - KEEP AS IS)
...
def safe_decimal(value):
    ...
    except (ValueError, TypeError, InvalidOperation) as e: logging.warning(f"Could not convert '{value}' to Decimal: {e}"); return None
...
class AnnualData(Base):
    ...
    # --- >>> NEW COLUMN DEFINITION ADDED TO MODEL <<< ---
...
# --- Processing Function --- (process_company_facts - KEEP AS IS)
# This function extracts individual data points; calculation happens later.
def process_company_facts(cik, company_data):
    ...
    for key, full_tag in DESIRED_TAGS.items():
        ...
        try:
            ...
            elif units: first_unit = list(units.keys())[0]; expected_unit_part = 'shares' if is_share_metric else ('USD/shares' if is_eps_metric else 'USD'); unit_key = first_unit; # Basic fallback
...
if __name__ == "__main__":
    ...
    try:
        ...
        Base.metadata.create_all(engine) # Creates table if not exists, does nothing if exists
    ...
    # --- Process Ticker List ---
    all_companies = {}
    try: # Process based on observed SEC format (dict) or list fallback
    ...
    for cik_str, company_info in companies_to_process:
        ...
        # --- ETA Calculation ---
        if processed_count > 10: # Show ETA after processing a few
            ...
            if annual_data_by_year:
                ...
                for year, data_for_year in sorted(annual_data_by_year.items()):
                    try:
                        ...
                        ocf = data_for_year.get('operating_cash_flow') # Already a Decimal or None
                        capex = data_for_year.get('capital_expenditures') # Already a Decimal or None
                        calculated_fcf = None # Default to None
                        if ocf is not None and capex is not None:
                            # Standard FCF: OCF - CapEx. Assumes CapEx (Payments...) is positive.
                            calculated_fcf = ocf - capex
                            logging.debug(f"{log_prefix} Year {year}: OCF={ocf}, CapEx={capex}, FCF={calculated_fcf}")
                        else:
                            logging.debug(f"{log_prefix} Year {year}: Cannot calculate FCF (OCF: {ocf}, CapEx: {capex})")
                        # --- >>> END FCF CALCULATION <<< ---

                        # Prepare data for ORM, including calculated FCF
                        record_data_for_orm = {
                            'ticker': ticker,
                            'company_name': effective_company_name,
                            'calculated_free_cash_flow': calculated_fcf, # Add the calculated value
                            **data_for_year # Unpack original data (will overwrite if keys overlap, safe here)
                        }
                        ...
                        error_count += 1; db_session.rollback() # Rollback on error
